-- 修复 search_candidates_rpc 函数中的 id 列歧义问题
-- 问题：函数参数名与表列名冲突导致的歧义

-- 重新创建 search_candidates_rpc 函数，明确所有列的来源
...
BEGIN
  ...
  WHERE 
    ...
    AND (user_id_param IS NULL OR r.owner_id = user_id_param)  -- 使用重命名的参数
...
-- 重新创建 search_jobs_rpc 函数，明确所有列的来源
...
BEGIN
  ...
  WHERE 
    ...
    AND (user_id_param IS NULL OR j.owner_id = user_id_param)  -- 使用重命名的参数
