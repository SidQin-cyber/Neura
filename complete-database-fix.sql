...
BEGIN
  ...
  RAISE NOTICE '';
  RAISE NOTICE '🔧 修复内容包括:';
  RAISE NOTICE '  1. 添加缺失的 has_embedding 字段';
  RAISE NOTICE '  2. 创建所有必要的搜索函数';
  RAISE NOTICE '  3. 修复 NULL owner_id 问题';
  RAISE NOTICE '  4. 修复空数组 [] 技能筛选问题';
  RAISE NOTICE '';
...
SELECT 
  ...
  CASE 
    WHEN COUNT(*) > 0 THEN '✅ 搜索功能正常' 
    ELSE '❌ 搜索功能异常' 
  END as status
FROM search_candidates_rpc(
  (SELECT embedding_text FROM test_vector),
