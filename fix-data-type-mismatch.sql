-- 修复数据类型不匹配问题
-- 将 FLOAT 类型改为 REAL 类型以匹配 PostgreSQL 的实际返回类型

-- 重新创建 search_candidates_rpc 函数，修复数据类型
...
RETURNS TABLE (
  ...
  languages          JSONB,
  status             TEXT,
  similarity         REAL,    -- 改为 REAL 类型
  fts_rank           REAL,    -- 改为 REAL 类型
  combined_score     REAL     -- 改为 REAL 类型
)
LANGUAGE plpgsql
AS $$
DECLARE
...
-- 重新创建 search_jobs_rpc 函数，修复数据类型
