-- 修复搜索函数用户ID过滤问题
-- 移除search_candidates_rpc函数中的owner_id过滤，实现数据共享
...
CREATE OR REPLACE FUNCTION search_candidates_rpc(
  ...
  user_id              UUID   DEFAULT NULL,  -- 保留参数但不使用
...
BEGIN
  ...
  SELECT 
    ...
    ts_rank(r.fts_document, query_tsquery)::FLOAT            AS fts_rank,
...
BEGIN
  ...
  SELECT 
    ...
    ts_rank(j.fts_document, query_tsquery)::FLOAT            AS fts_rank,
...
-- 验证修复
SELECT 'search_candidates_rpc function updated successfully' as status; 